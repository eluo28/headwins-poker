...
class StartingDataCommands(commands.Cog):
    ...
    async def upload_starting_data(
        ...
        try:
            ...
            success, message = await self.s3_service.upload_starting_data(starting_data_file, str(interaction.guild_id))
    ...
    async def list_starting_data(self, interaction: discord.Interaction) -> None:
        try:
            await interaction.response.defer(thinking=True)
            logger.info(f"Listing starting data files for guild {interaction.guild_id}")

            files, message = await self.s3_service.list_files(str(interaction.guild_id), "starting_data", limit=10)
    ...
    async def delete_starting_data(
        ...
        logger.info(f"Deleting starting data file {filename} for guild {interaction.guild_id}")
        try:
            await interaction.response.defer(thinking=True)
            logger.info(f"Deleting starting data file {filename} for guild {interaction.guild_id}")

            # First check if file exists
            files, _ = await self.s3_service.list_files(str(interaction.guild_id), "starting_data")
            ...
            success, message = await self.s3_service.delete_file(str(interaction.guild_id), filename, "starting_data")
            await interaction.followup.send(message, ephemeral=not success)
